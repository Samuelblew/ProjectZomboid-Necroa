-- 🔹 ForceZedWear - Versão Masculina (OnTick + Hysteresis)
local ModState = require("ModState")
local SM = ScriptManager.instance

-- Configurações
local ADD_THRESHOLD   = 12   -- ativa
local REMOVE_THRESHOLD = 21  -- desativa
...
local RULES = {
    { FULLTYPE = "FZ3.MZ3",  TEXTURES = { "M_ZedBody01_level1", "M_ZedBody01_level2", "M_ZedBody01_level3" } },
    { FULLTYPE = "FZ3.MZ31", TEXTURES = { "M_ZedBody02_level1", "M_ZedBody02_level2", "M_ZedBody02_level3" } },
    { FULLTYPE = "FZ3.MZ32", TEXTURES = { "M_ZedBody04_level1", "M_ZedBody04_level2", "M_ZedBody04_level3" } },
    { FULLTYPE = "FZ3.MZ33", TEXTURES = { "M_ZedBody03_level1", "M_ZedBody03_level2", "M_ZedBody03_level3" } },
}
...
-- Atualiza máscara de um zumbi masculino
local function updateZombieMaskVisuals(z, activeIcons)
    if not z or z:isDead() or z:isFemale() then return end
...
-- Atualiza zumbis masculinos próximos
local function updateZombiesAroundPlayer(player)
    ...
    for i = 0, zombies:size()-1 do
        ...
        if z and not z:isDead() and not z:isFemale() then
