...
SusceptibleMod.calculateThreat = function (player)
	-- MODIFICAÇÃO INSERIDA AQUI --
	-- Este bloco verifica se a infecção por cadáveres está habilitada pelo seu outro script.
	-- Se 'AirINF' não existir ou a variável 'corpseInfectionEnabled' não for 'true',
	-- ele executa a função original (sem a lógica dos corpos) e encerra.
	if not AirINF or AirINF.corpseInfectionEnabled ~= true then
		return susceptibleCorpses.oldCalculateThreat(player)
	end
	-- FIM DA MODIFICAÇÃO --
